...
const BASE = process.env.PUBCHEM_API_URL ?? "https://pubchem.ncbi.nlm.nih.gov/rest/pug";
...
async function findCidByName(name: string): Promise<string | null> {
  const res = await fetch(`${BASE}/compound/name/${encodeURIComponent(name)}/cids/JSON`);
  if (!res.ok) return null;
  const data = await res.json();
  return data?.IdentifierList?.CID?.[0]?.toString() ?? null;
}

const NA = null; // renders as "TIDAK DITEMUKAN" in the UI layer
...
export async function getPubChemProfile(query: {
  ...
  if (!cid && query.name) {
    cid = await findCidByName(query.name);
  }
  if (!cid) {
    return { error: "TIDAK DITEMUKAN — compound not found in PubChem." };
  }

  const propRes = await fetch(`${BASE}/compound/cid/${cid}/property/${PROPERTY_LIST}/JSON`);
  if (!propRes.ok) {
    return { error: `PubChem request failed (${propRes.status})` };
  }
...
export async function searchPubChemCandidates(name: string): Promise<
  Array<{ cid: string; title: string }>
> {
  ...
  const titleRes = await fetch(`${BASE}/compound/cid/${top.join(",")}/property/Title/JSON`);
